...
router.get('/', function (req, res) {
  ...
  `;
    ...
    dbConn.query(countQuery, function (err, countResult) {
        if (err) {
            req.flash('error', err);
            return res.render('dashboard/publishers', {
                data: [],
                currentPage: 1,
                totalPages: 1,
                messages: req.flash()
        ...
        dbConn.query(dataQuery, [limit, offset], function (err, rows) {
            ...
            res.render('dashboard/publishers', {
                data: rows,
                currentPage: page,
                totalPages,
                messages: req.flash()
...
router.get('/add', function (req, res, next) {
    res.render('dashboard/publishers/add', {
        name: '',
        state: 1,
        messages: req.flash()
    });
...
router.post('/add', function (req, res, next) {
    ...
    dbConn.query('INSERT INTO publishers SET ?', form_data, function (err, result) {
        if (err) {
            ...
            res.redirect('dashboard/publishers/add');
        } else {
            req.flash('success', 'Editorial agregada exitosamente');
            res.redirect('/dashboard/publishers');
        }
    });
});
...
router.get('/edit/:id', function (req, res, next) {
    ...
    dbConn.query('SELECT * FROM publishers WHERE id_publisher = ?', [id], function (err, publishers) {
        ...
        if (publishers.length <= 0) {
            ...
            return res.redirect('/dashboard/publishers');
        }

        const publisher = publishers[0];

        res.render('dashboard/publishers/edit', {
...
router.post('/update/:id', function (req, res, next) {
    ...
    if (errors) {
        return res.render('dashboard/publishers/edit', {
            title: 'Editar Editorial',
            publisher: {
                id_publisher: id,
                name,
